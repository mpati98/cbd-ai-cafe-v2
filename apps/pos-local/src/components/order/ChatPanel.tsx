...
export default function ChatPanel({
  ...
  /** Khi có (chỉ dùng ở sheet mobile) — hiện nút đóng cạnh nút đọc to trong header. */
  onClose?: () => void;
}) {
  ...
  useEffect(() => {
    ...
    if (last && last.from === "bot" && last.id !== lastSpokenId.current) {
      ...
      speak(last.text.replace(/\*\*/g, ""), tableCode);
    }
  ...
  async function submit(value: string) {
    ...
    try {
      const res = await fetch("/api/order-chat", {
        ...
        body: JSON.stringify({ history, message: trimmed, tableLabel: tableLabel ?? null, tableCode: tableCode ?? null }),
      });
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {onClose && (
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full border border-latte-700 text-latte-400 hover:border-latte-500 hover:text-latte-100"
            >
              ✕
            </button>
          )}
