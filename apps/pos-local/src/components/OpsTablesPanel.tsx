...
import { TableRow } from "@/types/ops";
...
export default function TablesPanel() {
  ...
  async function load() {
    setLoadError(null);
    try {
      const data = await opsApi.list<TableRow>("/api/tables");
      setTables(data);
    } catch (err) {
      setTables([]);
      setLoadError(err instanceof OpsApiError ? err.message : "Không tải được danh sách bàn.");
    }
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      await opsApi.create("/api/tables", { label: newLabel.trim() });
      setNewLabel("");
      await load();
    } catch (err) {
      window.alert(err instanceof OpsApiError ? err.message : "Không tạo được bàn.");
    } finally {
      setCreating(false);
    }
  }
  ...
  async function toggleActive(t: TableRow) {
    try {
      await opsApi.update(`/api/tables/${t.id}`, { isActive: !t.isActive });
      await load();
    } catch (err) {
      window.alert(err instanceof OpsApiError ? err.message : "Không cập nhật được.");
    }
  }
  ...
  async function handleRename(e: React.FormEvent) {
    ...
    try {
      await opsApi.update(`/api/tables/${renaming.id}`, { label: renameValue.trim() });
      setRenaming(null);
      await load();
    } catch (err) {
      window.alert(err instanceof OpsApiError ? err.message : "Không đổi tên được.");
    }
  }
  ...
  async function handleDelete(t: TableRow) {
    ...
    try {
      await opsApi.remove(`/api/tables/${t.id}`);
      await load();
    } catch (err) {
      window.alert(err instanceof OpsApiError ? err.message : "Không xoá được bàn.");
    }
  }
