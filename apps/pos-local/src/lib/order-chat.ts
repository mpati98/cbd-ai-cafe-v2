...
import { buildKnowledgeContextBlock, recordTopicAsk, selectTopicsForQuery } from "@/lib/knowledge-cache";
import { getSystemPromptTemplate } from "@/lib/system-prompt-cache";
...
/** `template` đồng bộ từ dashboard (SystemPromptConfig, xem lib/system-prompt-cache.ts)
 * — 2 placeholder bắt buộc: {{TABLE_LINE}} (dòng "khách ngồi bàn X", hoặc rỗng)
 * và {{MENU_BLOCK}} (danh sách món hiện có). */
function buildSystemPrompt(template: string, menuBlock: string, tableLabel: string | null | undefined): string {
  return template
    .replace("{{TABLE_LINE}}", tableLabel ? `- Khách đang ngồi tại bàn "${tableLabel}".` : "")
    .replace("{{MENU_BLOCK}}", menuBlock)
    .trim();
}
...
async function fetchKnowledgeBlock(message: string): Promise<string> {
  try {
    ...
    void Promise.all(topics.map((t) => recordTopicAsk(t.id))).catch(() => {});
...
export async function runOrderChat(input: {
  ...
  // Giới hạn tài nguyên AI theo bàn (nếu có quét QR) — xem lib/table-session.ts.
  // Không có tableCode (đặt món không qua bàn, vd mang về) thì bỏ qua, chỉ
  // còn rate-limit chung ở route.ts.
  const session = input.tableCode ? await getOrCreateTableSession(input.tableCode) : null;
  if (session) assertTokenQuota(session);

  const [rawItems, knowledgeBlock, template] = await Promise.all([
    getMenuItems(),
    fetchKnowledgeBlock(message),
  ...
  const system: Anthropic.TextBlockParam[] = [
    {
      type: "text",
      text: buildSystemPrompt(template, buildMenuBlock(items), input.tableLabel),
  ...
  // Trừ token THẬT sau khi đã có toàn bộ response (kể cả nhiều vòng tool-use)
  // — best-effort, không chặn trả lời cho khách nếu ghi quota lỗi.
  if (session) {
    void deductTokens(session.id, totalTokens).catch((err) => {
      console.warn("[order-chat] Không trừ được quota token:", err instanceof Error ? err.message : err);
    });
  }
