...
export function requireDb() {
  if (!prisma) {
    throw new ApiError(
      503,
      "Database chưa được cấu hình. Đặt DATABASE_URL/DIRECT_URL trong .env rồi chạy `npm run db:push:cloud`."
    );
  }
  return prisma;
}
...
export function withErrorHandling<A extends unknown[]>(
  ...
  return async (...args: A): Promise<NextResponse> => {
    try {
      ...
      // Bảng chưa tồn tại trên DB (đã sửa schema.cloud.prisma nhưng chưa `db:push:cloud`)
      if (typeof err === "object" && err !== null && "code" in err && (err as { code?: string }).code === "P2021") {
        return NextResponse.json(
          {
            ok: false,
            error:
              "Bảng chưa tồn tại trên database. Schema đã có model mới nhưng chưa được đẩy lên Neon — chạy `npm run db:push:cloud` rồi thử lại.",
          },
          { status: 500 }
        );
      }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Thiếu cột trên database (schema có field mới nhưng chưa đẩy lên Neon) — chạy `npm run db:push:cloud` rồi thử lại.",
          },
          { status: 500 }
        );
      }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Prisma Client chưa khớp với schema mới nhất. Chạy `npm run generate:cloud --workspace=@cbd/database` (và `npm run db:push:cloud` nếu có bảng mới) rồi khởi động lại server.",
          },
          { status: 500 }
        );
      }
